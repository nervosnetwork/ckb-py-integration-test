...
class TestCkbExportImport(CkbTest):
        ...
        if not os.path.exists(expected):
            ...
            if len(files) == 1:
                ...
                raise FileNotFoundError(
                    f"ckb export produced no .jsonl file in {range_dir}"
                )
            else:
                raise RuntimeError(
                    f"ckb export produced multiple .jsonl files in {range_dir}: {files}"
                )
        return expected
    ...
    def test_10_import_num_threads(self):
        ...
        try:
            run_command(
                f"cd {target.ckb_dir} && ./ckb import --num-threads 2 {export_file}"
            )
            target.start()
            tip = target.getClient().get_tip_block_number()
            assert tip == 50
